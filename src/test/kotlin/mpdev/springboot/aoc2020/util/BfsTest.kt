...
class BfsTest {
    ...
    fun `Builds Graph and Converts to String`() {
        val graph = graph1()
        val bfs = Bfs<Int>()
        println(bfs.graphToString(graph, graph[0]))
    ...
    @Test
    fun `Finds Shortest Path (Graph)`() {
        val graph = graph1()
        val bfs = Bfs<Int>()
        val path = bfs.shortestPath(graph[0], graph[7]).map { it.getId() }
        println(path)
        assertThat(path).isEqualTo(listOf(0,2,7))
    }
    ...
    fun `Finds All Paths (Graph)`() {
        val graph = graph1()
        val bfs = Bfs<Int>()
        val paths = bfs.allPaths(graph[0], graph[7]).map { it.map { n -> n.getId() } }
